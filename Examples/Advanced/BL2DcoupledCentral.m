...
Cdp = 0.3; % Dykstra-Parsons coef.
[k, phi] = RandPermField(k0, phi0, Cdp, 10, 5, Nx, Ny); % Random perm  field
krw0 = 1;
kro0 = 1;
nw = 4;
no = 2;
...
lw = harmonicMean(m, k)/mu_water;
lo = harmonicMean(m, k)/mu_oil;
Lw = @(sw)(krw(sw));
...
%     end
    ...
    while ((error_p>eps_p) || (error_sw>eps_sw))
        ...
        pgrad = gradientTerm(m, p.value);
        sw_face = arithmeticMean(m, sw.value); % average value of water saturation
        labdao = lo.*funceval(kro, sw_face);
        labdaw = lw.*funceval(krw, sw_face);
        ...
        Mconvsw1 = convectionTerm(m, -dlabdadsw.*pgrad);
        Mconvsw2 = convectionTerm(m, -dlabdawdsw.*pgrad);
        [Mtranssw2, RHStrans2] = transientTerm(m, phi, dt, sw);
        % Compute RHS values
    ...
    p.Old = p.value;
    sw.Old = sw.value;
    figure(1);visualizeCells(m, sw.value);
end